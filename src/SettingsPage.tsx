...
export default function SettingsPage({
  ...
  const [dailyReport,setDailyReport]=useState(()=>{try{return JSON.parse(localStorage.getItem("printmanager.daily-report")||"null")||{enabled:false,recipient:"",sendTime:"18:00",serviceId:"",templateId:"",publicKey:""}}catch{return {enabled:false,recipient:"",sendTime:"18:00",serviceId:"",templateId:"",publicKey:""}}});
      ...
      <section className="settings-grid">
        <div>
          ...
          <section className="settings-card daily-report-settings">
            ...
            <div className="report-form-grid"><label>CEO email address<input type="email" placeholder="ceo@company.com" value={dailyReport.recipient} onChange={event=>setDailyReport({...dailyReport,recipient:event.target.value})}/></label><label>Send time<input type="time" value={dailyReport.sendTime} onChange={event=>setDailyReport({...dailyReport,sendTime:event.target.value})}/></label></div><div className="report-form-grid report-provider-fields"><label>EmailJS service ID<input placeholder="service_xxxxx" value={dailyReport.serviceId||""} onChange={event=>setDailyReport({...dailyReport,serviceId:event.target.value})}/></label><label>EmailJS template ID<input placeholder="template_xxxxx" value={dailyReport.templateId||""} onChange={event=>setDailyReport({...dailyReport,templateId:event.target.value})}/></label></div><label>EmailJS public key<input placeholder="Your public key" value={dailyReport.publicKey||""} onChange={event=>setDailyReport({...dailyReport,publicKey:event.target.value})}/></label>
