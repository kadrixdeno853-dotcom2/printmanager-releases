...
export default function SetupWizard({ onComplete }: Props) {
  ...
  return <div className="setup-page">
    <div className="setup-brand"><CompanyLogo className="brand-mark"/><strong>PrintAcad</strong></div>
    <div className="setup-card">
      ...
      {step === 0 && <form onSubmit={next} className="setup-content">
        <div className="setup-symbol"><Building2 size={25} /></div><p className="eyebrow">WELCOME TO PRINTACAD</p><h1>Lets set up your business</h1><p className="setup-copy">This information will appear on your quotations, invoices and receipts. You can change it later.</p>
        ...
        <div className="form-row"><label>Business phone<input required value={profile.phone} onChange={e => update("phone", e.target.value)} placeholder="+256 7XX XXX XXX" /></label><label>TIN <small>(optional)</small><input value={profile.tin} onChange={e => update("tin", e.target.value)} placeholder="Tax identification number" /></label></div>
      ...
      {step === 1 && <form onSubmit={next} className="setup-content">
        <div className="setup-symbol"><ShieldCheck size={25} /></div><p className="eyebrow">OWNER ACCOUNT</p><h1>Who manages this business?</h1><p className="setup-copy">Youll be the first owner. Secure sign-in credentials will be added in the user-account step.</p>
      ...
      {step === 2 && <div className="setup-content">
        <div className="setup-symbol"><HardDrive size={25} /></div><p className="eyebrow">DATA PROTECTION</p><h1>Your work stays on this computer</h1><p className="setup-copy">PrintAcad works without internet. It will keep automatic local backups, while optional online backup can be connected later.</p>
