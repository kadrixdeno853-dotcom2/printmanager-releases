...
export default function JobsPage({
  ...
  const printReceipt = (job: Job) => {
    ...
    w.document.write(`<html><head><title>Receipt ${job.jobNumber}</title><style>body{font:14px Arial;padding:36px;color:#20312a}h1{color:#277d54}table{width:100%;margin-top:20px}td{padding:8px;border-bottom:1px solid #ddd}td:last-child{text-align:right}</style></head><body><h1>PrintAcad</h1><h2>Receipt ${job.jobNumber}</h2><p>Customer: ${job.customerName || "Walk-in customer"}</p><table><tr><td>${job.title}</td><td>UGX ${job.totalAmount.toLocaleString("en-UG")}</td></tr></table><h2 style="text-align:right">Total: UGX ${job.totalAmount.toLocaleString("en-UG")}</h2><p>Thank you for your business.</p></body></html>`); w.document.close(); w.print();
  };
  ...
  const persistJob = async () => {
    ...
    try {
      ...
      setEditing(null);
      if (!wasEditing) setReceipt(saved);
      await load();
    } catch (reason) {
    ...
    <>
      <section className="list-heading">
        <div>
          ...
          <p>
            {board
              ? "See every jobs current stage and move work through production."
